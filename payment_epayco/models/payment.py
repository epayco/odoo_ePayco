...
class PaymentAcquirerPayco(models.Model):
    ...
    provider = fields.Selection(selection_add=[('payco', 'Payco')], ondelete={'payco': 'set default'})
...
class PaymentTransactionPayco(models.Model):
    ...
    def _payco_form_validate(self, data):
        ...
        if float_compare(float(data.get('x_amount', '0.0')), tx.amount, 2) == 0:
            ...
            if isTestPluginMode == "no":
                if x_approval_code != "000000" and int(cod_response) == 1:
                    validation = True
                else:
        ...
        if shasign_check == True and validation == True:
            if tx.state not in ['draft', 'done']:
                if cod_response not in [1, 3]:
                    self.upload_message_order('cancel')
                    self.manage_status_order(data.get('x_extra1'), 'sale_order', 'cancel')
                else:
                    if tx.state in ['pending']:
                        if cod_response == 1:
                            self._set_transaction_done()
                        if cod_response in [3]:
                            self._set_transaction_pending()
                    if tx.state in ['cancel']:
                        if cod_response == 1:
                            self.upload_message_order('done')
                            self.manage_status_order(data.get('x_extra1'), 'sale_order', 'draft')
                            self._set_transaction_done()
                        if cod_response == 3:
                            self.upload_message_order('pending')
                            self.manage_status_order(data.get('x_extra1'), 'sale_order', 'draft')
                            self._set_transaction_pending()
            elif tx.state not in ['done', 'pending']:
                if cod_response == 1:
                    self._set_transaction_done()
                elif cod_response in [3]:
                    self._set_transaction_pending()
                else:
                    self.manage_status_order(data.get('x_extra1'), 'sale_order', 'cancel')
            ...
            if tx.state in ['done']:
                self.upload_message_order('cancel')
                self.manage_status_order(data.get('x_extra1'), 'stock_picking', 'cancel', confirmation=True)
                self.manage_status_order(data.get('x_extra1'), 'sale_order', 'cancel')
                self.manage_status_order(data.get('x_extra1'), 'stock_move', 'cancel', confirmation=True)
            else:
                self.manage_status_order(data.get('x_extra1'), 'sale_order', 'cancel')
    ...
    def manage_status_order(self, order_name, model_name, state, confirmation=False):
        condition = self.reflect_params(order_name, confirmation)
        params = {'state': 'draft'}
        self.query_update_status(model_name, params, condition)
        self.query_update_status(model_name, {'state': state}, condition)
        if state == 'cancel':
            self._set_transaction_cancel()

    def upload_message_order(self, state):
        allowed_states = ('draft', 'authorized', 'pending')
        target_state = 'draft'
        (tx_to_process, tx_already_processed, tx_wrong_state) = self._filter_transaction_state(allowed_states,
                                                                                               target_state)
        massage = "ePayco confirm"
        tx_to_process.write({
            'state': target_state,
            'date': fields.Datetime.now(),
