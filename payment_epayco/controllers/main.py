...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    @http.route(
        '/epayco/payment/transaction/<int:invoice_id>', type='json', auth='public', website=True
    )
    def epayco_payment_transaction(self, invoice_id, access_token, **kwargs):
    ...
    @http.route(
        '/epayco/redirect/backend', type='http', auth='public', website=True, csrf=False, save_session=False
    )
    def epayco_backend_redirec(self, **post):
        return self._epayco_process_response(post)

    def _epayco_process_response(self, data):
    ...
    @http.route(
            '/epayco/confirmation/backend', type='http', auth='public', website=True, csrf=False, save_session=False
        )
    def epayco_confirmation_redirec(self, **post):
        request.env['payment.transaction'].sudo()._handle_feedback_data('epayco', post)
        return request.redirect('/payment/status')
...
class EpaycoController(http.Controller):
    ...
    @http.route('/payment/epayco/transction/process', type='http', auth='public', methods=['GET', 'POST'], csrf=False)
    def epayco_transction_process(self, **data):
        try:
            if not data.get('reference'):
                raise ValidationError("ePayco: " + _("Transaction not Created"))
            ...
            res = self.epayco_create_indention(tx_sudo, acquirer_sudo , **data)
            if not res:
                raise ValidationError("ePayco: " + _("Transaction not Created"))
    ...
    def epayco_create_indention(self, tx_id=None, acquirer_sudo=None, **data):
        ...
        external = 'true' if acquirer_sudo.epayco_checkout_type == 'standard' else 'false'

        payload = {
            "public_key": acquirer_sudo.epayco_public_key,
